...
# Computed as: union of all source fields minus the target type's valid fields.
my %STALE_FIELDS = do {
    my %valid_per_type = map {
        my $type   = $_;
        my %fields = map { $_ => 1 } @{ $TYPE_FIELDS{$type} };
        $type => \%fields;
    } keys %TYPE_FIELDS;
    map {
        my $type = $_;
        $type => [ grep { !$valid_per_type{$type}{$_} } @SOURCE_FIELDS ];
    } keys %TYPE_FIELDS;
};
...
# Transport details are delegated to fetch_registry_resource.
